    ...
    const fetchData = async () => {
      try {
        ...
        if (productRes.ok && productData.product) {
          ...
          setUpiPrice(p.upiPrice?.toString() || "");
          setCreditCardPrice(p.creditCardPrice?.toString() || "");
          ...
          setWarrantyTitle(p.warrantyTitle || "1 Year Warranty");
          setWarrantySubtitle(p.warrantySubtitle || "Full coverage");
          setDeliveryTitle(p.deliveryTitle || "Free Delivery");
          setDeliverySubtitle(p.deliverySubtitle || "Pan India");
          setReturnsTitle(p.returnsTitle || "7-Day Returns");
          setReturnsSubtitle(p.returnsSubtitle || "Easy returns");
          setSupportTitle(p.supportTitle || "Expert Support");
          setSupportSubtitle(p.supportSubtitle || "24/7 available");
    ...
    try {
      ...
      const res = await fetch(`/api/admin/products/${id}`, {
        ...
        body: JSON.stringify({
          ...
          sku: sku || null, price: parseFloat(price), upiPrice: parseFloat(upiPrice), creditCardPrice: parseFloat(creditCardPrice),
          ...
          <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                  <Label>UPI Price (Rs.) *</Label>
                  <Input type="number" className="mt-1.5" value={upiPrice} onChange={(e) => setUpiPrice(e.target.value)} />
                  <p className="text-xs text-muted-foreground mt-1">Price for UPI payments</p>
                </div>
                <div>
                  <Label>Credit Card Price (Rs.) *</Label>
                  <Input type="number" className="mt-1.5" value={creditCardPrice} onChange={(e) => setCreditCardPrice(e.target.value)} />
                  <p className="text-xs text-muted-foreground mt-1">Price for card payments</p>
                </div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  <Label>MRP / Actual Price (Rs.)</Label>
