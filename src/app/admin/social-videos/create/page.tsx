...
export default function CreateSocialVideoPage() {
  ...
  // Fetch badges, products, and prebuilt PCs
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const [badgesRes, productsRes, prebuiltRes] = await Promise.all([
  ...
  // Auto-extract YouTube thumbnail when URL changes
  useEffect(() => {
    if (platform === "YOUTUBE" && videoUrl && !thumbnailUrl) {
      const patterns = [
  ...
  }, [videoUrl, platform, thumbnailUrl]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Add a new video from YouTube, Instagram, or Facebook"
        actions={
          ...
          <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Leave empty to auto-extract from YouTube videos
                </p>
              </div>
                          ...
                          {products.map((product) => (
                            <CommandItem
                              ...
                              onSelect={() => {
                                ...
                                setPrebuiltPCId(""); // Clear prebuilt if product selected
                          ...
                          {prebuiltPCs.map((pc) => (
                            <CommandItem
                              ...
                              onSelect={() => {
                                ...
                                setProductId(""); // Clear product if prebuilt selected
