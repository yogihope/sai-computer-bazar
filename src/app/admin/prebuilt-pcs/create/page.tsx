...
export default function CreatePrebuiltPCPage() {
  ...
  // Fetch PC types, tags, and badges
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [pcTypesRes, tagsRes, badgesRes] = await Promise.all([
          fetch("/api/admin/pc-types"),
          fetch("/api/admin/tags"),
          fetch("/api/admin/badges"),
        ]);
        if (pcTypesRes.ok) {
          const data = await pcTypesRes.json();
          setPcTypes(data.pcTypes || []);
        }
        if (tagsRes.ok) {
  ...
  const addComponent = (product: SearchProduct) => {
    if (components.some(c => c.productId === product.id)) {
      toast({ title: "Already Added", description: "This product is already in the bundle", variant: "destructive" });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        pcTypeId: selectedPcTypeId || null,
        targetUse: targetUse || null,
      ...
      if (res.ok) {
        toast({ title: "Success", description: "Prebuilt PC created successfully" });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Create Prebuilt PC"
        subtitle="Build a new PC bundle by adding products"
        actions={
          <div className="flex items-center gap-3">
            <Link href="/admin/prebuilt-pcs">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>PC Name *</Label>
                <Input placeholder="e.g., Gaming Beast Pro RTX 4090" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Slug *</Label>
                <Input placeholder="gaming-beast-pro" value={slug} onChange={(e) => setSlug(e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input placeholder="e.g., For 4K Gaming" value={targetUse} onChange={(e) => setTargetUse(e.target.value)} />
              </div>
            </div>

            <div className="space-y-2">
              <Label>Short Description</Label>
              <Textarea placeholder="Brief description..." value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} rows={2} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="Detailed specs..." value={specifications} onChange={(e) => setSpecifications(e.target.value)} rows={4} />
            </div>
          </div>
          ...
          <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => setProductSearchOpen(true)} className="gap-2">
            ...
            {components.length === 0 ? (
              <div className="text-center py-12 border-2 border-dashed border-border rounded-lg">
                ...
                <Button onClick={() => setProductSearchOpen(true)} variant="outline" className="gap-2">
                  <Plus className="w-4 h-4" />
                  Add First Component
                </Button>
              </div>
              ...
              <div className="space-y-3">
                {components.map((component) => (
                  <div key={component.id} className="flex items-center gap-4 p-4 bg-muted/30 rounded-lg border border-border/50">
                    <GripVertical className="w-5 h-5 text-muted-foreground" />
                    <div className="w-14 h-14 rounded-lg bg-muted flex items-center justify-center overflow-hidden">
                      {component.product.images?.[0]?.url ? (
                        <img src={component.product.images[0].url} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <Package className="w-6 h-6 text-muted-foreground" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{component.product.name}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="outline" className="text-xs">{component.componentType}</Badge>
                        <span className="text-sm text-muted-foreground">₹{parseFloat(component.product.price).toLocaleString("en-IN")}</span>
                      </div>
                    </div>
                    <Select value={component.componentType} onValueChange={(value) => updateComponent(component.id, { componentType: value })}>
                      <SelectTrigger className="w-32"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {COMPONENT_TYPES.map((type) => <SelectItem key={type} value={type}>{type}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <div className="flex items-center gap-2">
                      <Label className="text-sm text-muted-foreground">Qty:</Label>
                      <Input type="number" min={1} value={component.quantity} onChange={(e) => updateComponent(component.id, { quantity: parseInt(e.target.value) || 1 })} className="w-16" />
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => removeComponent(component.id)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
          ...
          <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-xl p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea value={seoDescription} onChange={(e) => setSeoDescription(e.target.value)} maxLength={170} rows={3} />
              <p className="text-xs text-muted-foreground">{seoDescription.length}/170</p>
            </div>

            <div className="space-y-2">
              <Label>Keywords</Label>
              <Input placeholder="gaming pc, rtx 4090" value={seoKeywords} onChange={(e) => setSeoKeywords(e.target.value)} />
            </div>
          </div>
        </div>
          ...
          <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-xl p-6 space-y-4">
            ...
            <div className="p-4 bg-card/50 rounded-lg border border-border/50">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-lg font-semibold">₹{totalPrice.toLocaleString("en-IN")}</span>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input type="number" className="pl-9" value={compareAtPrice || ""} onChange={(e) => setCompareAtPrice(parseFloat(e.target.value) || null)} />
              </div>
            </div>
            ...
            {totalPrice > 0 && sellingPrice > 0 && sellingPrice < totalPrice && (
              <div className="p-4 bg-emerald-500/10 rounded-lg border border-emerald-500/20">
                ...
                <div className="text-2xl font-bold text-emerald-500">
                  ₹{savings.toLocaleString("en-IN")} ({discountPercent}% OFF)
                </div>
              </div>
            )}
          </div>
                  ...
                  <CommandList>
                    <CommandEmpty>
                      <div className="p-2">
                        ...
                        {tagSearch && (
                          <Button size="sm" onClick={addNewTag} className="w-full gap-2">
                  ...
                  <CommandList>
                    <CommandEmpty>
                      <div className="p-2">
                        ...
                        {badgeSearch && (
                          <Button size="sm" onClick={addNewBadge} className="w-full gap-2">
                            <Plus className="w-3 h-3" />
                            Add "{badgeSearch}"
                          </Button>
                        )}
      ...
      <Dialog open={productSearchOpen} onOpenChange={setProductSearchOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>Search and add products to this PC</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-auto mt-4 -mx-6 px-6">
            {searchResults.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">{searchQuery ? "No products found" : "Search for products"}</p>
              </div>
            ) : (
              <div className="space-y-2">
                  ...
                  return (
                    <div key={product.id} className={`flex items-center gap-3 p-3 rounded-lg border ${isAdded ? "bg-primary/5 border-primary/20" : "hover:bg-muted/50 border-border/50"}`}>
                      <div className="w-12 h-12 rounded-lg bg-muted flex items-center justify-center overflow-hidden">
                        {product.images?.[0]?.url ? <img src={product.images[0].url} alt="" className="w-full h-full object-cover" /> : <Package className="w-5 h-5 text-muted-foreground" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium truncate">{product.name}</p>
                        <p className="text-sm text-muted-foreground">{product.primaryCategory.name} {product.brand && `• ${product.brand}`}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold">₹{parseFloat(product.price).toLocaleString("en-IN")}</p>
                        <p className={`text-xs ${product.isInStock ? "text-emerald-500" : "text-destructive"}`}>{product.isInStock ? "In Stock" : "Out of Stock"}</p>
                      </div>
                      <Button size="sm" variant={isAdded ? "secondary" : "default"} onClick={() => !isAdded && addComponent(product)} disabled={isAdded}>
                        {isAdded ? "Added" : "Add"}
                      </Button>
                    </div>
                  );
